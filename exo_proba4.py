...
for i in range(3):
    cardBob=random.choice(cards)
    cards.remove(cardBob)
    main_Bob.append(cardBob)
...
#je crée un nouveau deck de carte
cards_new = []
...
for i in range(0, 4):
    ...
    for j in range(1, 14):
        ...
        cards_new.append((symbol, color, j))
...
for manche in range(3):
    ...
    print("alice")
    for card in range(3):
        cardAl=random.choice(cards_new)
        print(cardAl)
        cards_new.remove(cardAl)
        main_alice.append(cardAl)
        nombresAL.append(cardAl[2])
        if cardAl[0]== "coeur":
            coeur +=1
        if card == 2 and (3 in nombresAL or coeur >=2):
            point_alice += 1
            print("alice prend 1 point")
            main_alice=[]
        cards_new.append(cardAl)
    main_bob=[]
    nombresBob=[]
    pique=0
    print("bob")
    for cardB in range(3):
        cardBob=random.choice(cards_new)
        print(cardBob)
        cards_new.remove(cardBob)
        main_bob.append(cardBob)
        nombresBob.append(cardBob[2])
        if cardBob[0]== "pique":
            coeur +=1
        if cardB == 2 and (7 in nombresBob or pique >=2):
            point_bob += 1
            print("Bob prend 1 point")
        cards_new.append(cardBob)
        main_bob=[]

print(f"alice: {point_alice}  |  Bob: {point_bob}")
if point_alice == point_bob:
    print("egalite")
elif point_alice<point_bob:
    print("bob win")
else:
    print("alice win")
